...
@click.command()
@click.option('-i', '--input_file', help='[INPUT] a path to an input file with Plasmidfinder results',
...
def main_program(status, input_file, output, error=""):
    if status != "tak":
        ...
        with open(input_file) as f:
            ...
            for line in f:
                ...
                if "Database" == line[0]:
                    ...
                    plasmid_name, seq_identity, coverage, contig_name, reference_name = line[1], int(float(line[2])), line[3], line[4], line[-1]
        ...
        json_output["plasmids_data"] = plasmids_data
