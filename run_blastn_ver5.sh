...
run_blastn () {
	if [ ${1} == 'STY1774' ]; then
		blastn -query ${2} -db /cgMLST2_entero/${1}.fasta -out blastn_tmp_${1}.tab -outfmt 6 -max_target_seqs 6 -word_size 5 
	else
		 blastn -query ${2} -db /cgMLST2_entero/${1}.fasta -out blastn_tmp_${1}.tab -outfmt 6 -max_target_seqs 6
	fi
	return 0
}
